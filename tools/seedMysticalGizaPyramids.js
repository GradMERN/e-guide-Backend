...
// Tour item texts and locations provided by the user (media is sourced from local `tools/tour 1`)
...
async function createTourAndItems(place, guide) {
  ...
  // Prepare audio candidates: prefer local audio files under TOUR_ASSETS_DIR (recursively).
  // Accept common audio/video container extensions so local recordings are picked up.
  ...
  // Prefer numbered audio files 0.mp3..19.mp3 (explicit ordering). If not all
  // present, include any other discovered audio files after the numbered ones.
  const numbered = [];
  for (let n = 0; n < 20; n++) {
  ...
  if (numbered.length === 20) {
    localAudioFiles = numbered;
  } else if (numbered.length > 0) {
    // keep numbered files first, then any other discovered files (deduped)
    localAudioFiles = [
      ...numbered,
  ...
  console.log("Discovered local audio files:", localAudioFiles);
  ...
  async function findAndUploadShortAudio(candidateSources) {
    ...
    for (const src of candidateSources) {
      try {
        // Determine extension/resource type. For common audio containers request
        // Cloudinary treat them as 'video' so duration is reliably returned.
        let ext = "";
        try {
          ext = path
  ...
  for (let i = 0; i < 20; i++) {
    ...
    if (audioObj) {
      ...
      try {
        const upa = await uploadToCloudinary(DEFAULT_AUDIO, "tours/giza/items");
        audioObj = {
